...
uint8_t Extracted_Data[SINGLE_PACKET_DATA_SIZE];
...
//  HAL_UART_Receive_IT(&huart2, (uint8_t*)uart_rx_buffer, sizeof(uart_rx_buffer));			   	// UART2 Interrupt Enable
  ...
  while(1)
  {
    ...
    /* USER CODE BEGIN 3 */
	 ...
	 if(GPS_FLAG)
	 {
		 Get_Gps_Coordinates();                                                           		// Update Global Variables Lati and Longi , and set the flag --> gpsDataReady
	 }
	 ...
	 if(buttonFlag)
	 {
	 	...
	 	if(GPS_READY)																// send the Emergency data only if the gps data is ready
	 	{
	 		sendEmergencyData();
	 	}
	 	else
